...
public class Reguser extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		mystore r=(mystore) getServletContext().getAttribute("reg");
		
		out.println("<html><head><title>User Registration Form</title></head><body>");
		out.println(" <form method='post' action='Reguser'>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//changing the register within qutoes to reg
		ArrayList<mystore> register = (ArrayList<mystore>) getServletContext().getAttribute("reg");
		ArrayList<mystore> st=new ArrayList<mystore>();
      //  mystore r=new mystore( "em", "pass", "ufn", "uln", "addst1", "addst2", "addcity", "addstate", "addzip", "pharea", "phprefix", "phsuffix"); 
		  mystore r=new mystore(); 
		PrintWriter out = response.getWriter();
    ...
    /*    if(!pass.isEmpty() && !pass.matches("((?=.*\\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#$%^&*]).{4,20})")){
        ...
        //	ArrayList<mystore> a=new ArrayList<mystore>();
        	mystore regis = new mystore(ufn, uln, pass,em, addst1, addst2, addcity, addzip,addstate, pharea, phprefix, phsuffix);
            st.add(regis);
            getServletContext().setAttribute("regist", st);
            response.sendRedirect("/Lab2CS320/lab3/Signin");
            out.flush();
        	}
        	else
        	{
		...
		//ArrayList<mystore> r1 = (ArrayList<mystore>) getServletContext().getAttribute("reg");
		   
	     out.println("You are successfully Registered now Login");
	    // out.println("<a href='/Lab2CS320/lab3/Signin'>Login Here</a>");
        response.sendRedirect("/cs320stu08/lab3/Signin");
        out.flush();
        	}
	}
        //reg.add(r);
